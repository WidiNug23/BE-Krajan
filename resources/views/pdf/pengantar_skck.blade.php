...
<body>
        ...
        <tr>
            ...
            <td class="title-cell center">
                ...
                <div class="kop-subtitle">
                    RUKUN TETANGGA {{ str_pad($skck->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) ?: '....' }}
                </div>
    ...
    <div class="center">
        ...
        <p class="nomor-surat">Nomor: {{ $skck->no_surat_pengantar ?? '......../......../........' }}</p>
    </div>

    <p class="indent">
        Yang bertanda tangan di bawah ini, kami Ketua RT {{ str_pad($skck->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) ?: '....' }} 
        ...
        <tr>
            ...
            <td class="ttd-container">
                ...
                <div style="margin-top: 5px;">
                    <div class="bold">Ketua RT {{ str_pad($skck->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) }}</div>
                    
                    <div class="ttd-image">
                        @php
                            $ttdRtPath = null;
                            if (!empty($skck->ttd_rt)) {
                                // kalau berupa URL -> ambil path setelah domain
                                if (Str::startsWith($skck->ttd_rt, ['http://', 'https://'])) {
                                    $ttdRtPath = public_path(parse_url($skck->ttd_rt, PHP_URL_PATH));
                                } else {
                                    // kalau sudah relative path
                                    $ttdRtPath = public_path($skck->ttd_rt);
                                }
                            }
                        @endphp
