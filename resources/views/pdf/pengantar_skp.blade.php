...
<body>
    ...
    <table class="kop-table">
        <tr>
            ...
            <td class="title-cell center">
                <div class="kop-title">PEMERINTAH KELURAHAN KRAJAN</div>
                <div class="kop-subtitle">
                    RUKUN TETANGGA {{ str_pad($skp->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) ?: '....' }}
                </div>
    ...
    <div class="center">
        ...
        <p class="nomor-surat">Nomor: {{ $skp->no_surat_pengantar ?? '......../......../........' }}</p>
    </div>

    <p class="indent">
        Yang bertanda tangan di bawah ini, kami Ketua RT {{ str_pad($skp->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) ?: '....' }} 
        ...
        <tr>
            ...
            <td class="ttd-container">
                ...
                <div style="margin-top: 5px;">
                    <div class="bold">Ketua RT {{ str_pad($skp->rt->no_rt ?? '', 2, '0', STR_PAD_LEFT) }}</div>
                    ...
                    <div class="ttd-image">
                        @php
                            ...
                            if (!empty($skp->ttd_rt)) {
                                ...
                                if (Str::startsWith($skp->ttd_rt, ['http://', 'https://'])) {
                                    $ttdRtPath = public_path(parse_url($skp->ttd_rt, PHP_URL_PATH));
                                } else {
                                    // kalau sudah relative path
                                    $ttdRtPath = public_path($skp->ttd_rt);
                                }
                            }
                        @endphp
