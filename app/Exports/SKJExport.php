    ...
    WithColumnFormatting
{
        ...
        public function headings(): array
    {
        return [
            ...
            'Keperluan', 'Nama Mantan Suami', 'Alasan', 'Keterangan', 'Status',
            'Nomor Surat', 'No Surat Pengantar RT', 'Poin II',
            'Validasi RT', 'Validasi Perangkat Desa', 'Validasi Kepala Desa',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $highestCol = $sheet->getHighestColumn(); // Ini akan otomatis mengambil sampai AF
                $highestRow = $sheet->getHighestRow();

                // 1. Style Header (Warna Biru sampai kolom terakhir AF)
                $sheet->getStyle("A1:{$highestCol}1")->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
                        'startColor' => ['rgb' => '1E40AF'] // Biru Bold
                    ],
                ]);

                // 2. Alignment & Wrap Text untuk semua sel data
                ...
                // 3. Tambahkan Border ke seluruh tabel
                $sheet->getStyle("A1:{$highestCol}{$highestRow}")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => Border::BORDER_THIN,
                            'color' => ['rgb' => 'D1D5DB'], // Border abu-abu halus
                        ],
                    ],
                ]);

                // 4. Baris Judul tetep di atas saat scroll (Freeze Pane)
                $sheet->freezePane('A2');

                // 5. Memberikan warna selang-seling (Zebra striping) agar mudah dibaca
