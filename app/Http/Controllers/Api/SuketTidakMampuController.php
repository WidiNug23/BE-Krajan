...
class SuketTidakMampuController extends Controller
{
    

 public function countSKTM(Request $request)
    {
        try {
            // ===============================
            // 1. VALIDASI TOKEN
            ...
            // ===============================
            $totalSKTM = SuketTidakMampu::count();
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil jumlah SKTM',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function index()
    {
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat mengambil data SKTM',
                'error' => $e->getMessage()
            ], 500);
        }
    }

    // GET DATA SKTM BY MASYARAKAT (TOKEN + VALIDASI ID) baru diperbaiki //TAMBAH
public function getByMasyarakat(Request $request)
{
    try {
        ...
        $paginator = SuketTidakMampu::where('masyarakat_id', $tokenMasyarakatId)
            ->with([
        ...
        // LOG DI SINI
        Log::info('SKTM PAGINATION DEBUG', [
        ...
        $collection = $paginator->getCollection()->map(function ($item) {
            ...
            $item->file_ktp = url("api/sktm/file/{$item->id_sktm}/ktp");
            $item->file_kk = url("api/sktm/file/{$item->id_sktm}/kk");
            
            // Perbaikan untuk Surat Pengantar RT jika file-nya tersedia
            if (!empty($item->file_pengantar_rt)) {
                $item->file_pengantar_rt = url("api/sktm/file/{$item->id_sktm}/pengantar");
            }

            // Tambahkan ini di dalam blok map() fungsi-fungsi GET Anda:
            if (!empty($item->file_pdf)) {
                $item->file_pdf = url("api/sktm/file/{$item->id_sktm}/pdf");
            }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar pengajuan SKTM masyarakat',
    ...
    // GET DATA SKTM BY KETUA RT TOKEN LOGIN
public function getByKetuaRT(Request $request)
{
    try {
        ...
        // Query + pagination
        $data = SuketTidakMampu::where('rt_id', $rt->id)
            ->with([
                'rt:id,nama,jabatan,role',
                'perangkat:id,nama,jabatan,role',
                'perangkatValidator:id,nama,jabatan,role',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar SKTM untuk RT yang sedang login',
            'pagination' => [
    ...
    // GET DATA SKTM BY PERANGKAT DESA TOKEN LOGIN
public function getByPerangkatDesa(Request $request)
{
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar SKTM untuk perangkat desa',
            'pagination' => [
                'current_page' => $data->currentPage(),
                'per_page'     => $data->perPage(),
                'total'        => $data->total(),
   ...
   // GET DATA SKTM BY KEPALA DESA TOKEN LOGIN
    public function getByKepalaDesa(Request $request)
    {
        try {
            ...
            // Kepala desa bisa melihat semua SKTM
            $data = SuketTidakMampu::with([
                    'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
            ...
            return response()->json([
                'success' => true,
                'message' => 'Daftar SKTM untuk kepala desa',
                'pagination' => [
                    'current_page' => $data->currentPage(),
                    'per_page'     => $data->perPage(),
                    'total'        => $data->total(),
...
// GET DATA SKTM BY SUPER ADMIN TOKEN LOGIN
    public function getAll(Request $request)
    {
        try {
            ...
            // Super admin bisa melihat semua SKTM
            $data = SuketTidakMampu::with([
                    'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
            ...
            return response()->json([
                'success' => true,
                'message' => 'Daftar SKTM (super admin)',
                'count' => $data->count(),
                'data' => $data
            ]);
    ...
    //TAMBAH
public function viewSecureFile(Request $request, $id_sktm, $type)
{
    try {
        // 1. Ambil data SKTM
        $sktm = SuketTidakMampu::find($id_sktm);
        if (!$sktm) {
        ...
        $isOwner = ($user && $user->tokenCan('masyarakat') && (int)$user->id === (int)$sktm->masyarakat_id);
        ...
        $dbPath = '';
        if ($type === 'ktp') {
            $dbPath = $sktm->file_ktp;
        } elseif ($type === 'kk') {
            $dbPath = $sktm->file_kk;
        } elseif ($type === 'pengantar') {
            $dbPath = $sktm->file_pengantar_rt;
        } elseif ($type === 'pdf') {
            $dbPath = $sktm->file_pdf; // 🔥 Mengarah ke kolom file_pdf terenkripsi
        }
    ...
    // POST DATA AJUAN SKTM BY MASYARAKAT //TAMBAH
   public function store(Request $request)
{
    ...
    $validated = $request->validate([
     ...
     if ((int) $validated['masyarakat_id'] !== (int) $masyarakat->id) {
        return response()->json([
            'success' => false,
            'message' => 'Anda tidak berhak mengajukan SKTM untuk masyarakat lain'
        ], 403);
    }
    ...
    if ($request->hasFile('file')) {
        $folder = public_path('uploads/sktm');
        ...
        $validated['file'] = url('uploads/sktm/' . $filename);
    }
    ...
    if ($request->hasFile('file_ktp')) {
        $folder = storage_path('app/private/sktm/ktp');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['file_ktp'] = 'private/sktm/ktp/' . $filename;
    }
    ...
    if ($request->hasFile('file_kk')) {
        $folder = storage_path('app/private/sktm/kk');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['file_kk'] = 'private/sktm/kk/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_masyarakat')) {
        $folder = public_path('uploads/sktm/ttd/masyarakat');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_masyarakat'] = 'uploads/sktm/ttd/masyarakat/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_rt')) {
        $folder = public_path('uploads/sktm/ttd/rt');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_rt'] = 'uploads/sktm/ttd/rt/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_kades')) {
        $folder = public_path('uploads/sktm/ttd/kades');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_kades'] = 'uploads/sktm/ttd/kades/' . $filename;
    }

        // Set default
        $validated['status'] = 'diproses rt';
        $validated['submitted_by'] = 'masyarakat';
        $validated['submitted_by_id'] = $masyarakat->id;
        ...
        $sktm = SuketTidakMampu::create($validated);

         // 🔔 NOTIFIKASI KE RT TUJUAN
    Notification::create([
        'user_id'    => $sktm->rt_id,
        'surat_type' => 'SKTM',
        'surat_id'   => $sktm->id_sktm,
        'title'      => 'Pengajuan surat keterangan tidak mampu baru',
        'message'    => 'Ada pengajuan surat keterangan tidak mampu baru dari masyarakat.',
    ]);

        return response()->json([
            'success' => true,
            'message' => 'Pengajuan surat keterangan tidak mampu berhasil dibuat',
            'data' => $sktm->load(['rt:id,nama,role,no_rt', 'masyarakat:id,nama,email,nik']),
        ], 201);
    }

    // POST DATA AJUAN SKTM BY KETUA RT
    public function storeByRT(Request $request)
{
   ...
   /// ===============================
    ...
    Notification::create([
        ...
        'surat_type' => 'SKTM',
        'surat_id'   => $sktm->id_sktm,
        'title'      => 'Pengajuan surat keterangan tidak mampu dari Ketua RT',
        'message'    => 'Ketua RT telah mengajukan dan memverifikasi surat keterangan tidak mampu.',
    ]);

    return response()->json([
        'success' => true,
        'message' => 'Pengajuan surat keterangan tidak mampu oleh Ketua RT berhasil',
        'data' => $sktm->load(['rt:id,nama,no_rt']),
    ], 201);
}
    ...
    if ($request->hasFile('file_kk')) {
       ...
       // =====================================================================
        ...
        if ($request->hasFile('file_pengantar_rt')) {
            $folder = storage_path('app/private/pengantar-rt-sktm');
            if (!file_exists($folder)) mkdir($folder, 0777, true);
            ...
            $validated['file_pengantar_rt'] = 'private/pengantar-rt-sktm/' . $filename;
        ...
        Notification::create([
            ...
            'surat_type' => 'SKTM',
            'surat_id'   => $sktm->id_sktm,
            'title' => 'Pengajuan surat keterangan tidak mampu Menunggu Verifikasi Perangkat Desa',
            'message' => 'Pengajuan surat keterangan tidak mampu dari perangkat desa menunggu verifikasi.',
        ]);
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pengajuan surat keterangan tidak mampu oleh perangkat desa berhasil',
            'data' => $sktm
        ], 201);
    }
    ...
    public function ajukanKembali(Request $request, $id)
{
    try {
        ...
        if (!$user) {
         ...
         // === Ambil SKTM ===
        $sktm = SuketTidakMampu::where('id_sktm', $id)->first();

        // ✅ FIX: cek dulu sebelum dipakai
        if (!$sktm) {
            return response()->json([
                'success' => false,
                'message' => 'Data SKTM tidak ditemukan'
            ], 404);
        }

            // === CEK HAK AKSES BERDASARKAN PENGAJU ===
        if ($sktm->submitted_by === 'masyarakat') {

            if (!$user->tokenCan('masyarakat') || $sktm->masyarakat_id != $user->id) {
                return response()->json([
                    'success' => false,
        ...
        } else if ($sktm->submitted_by === 'rt') {

            if (!$user->tokenCan('rt') || $sktm->rt_id != $user->id) {
                return response()->json([
                    'success' => false,
        ...
        } else if ($sktm->submitted_by === 'perangkat_desa') {
    ...
    // 🔥 FIX DI SINI
    if (!$request->hasFile('file_pengantar_rt') && !$sktm->file_pengantar_rt) {
        return response()->json([
            'success' => false,
    ...
    $validated['rt_validated_at'] = now();
        ...
        if (!$sktm) {
            return response()->json([
                'success' => false,
                'message' => 'Data Surat keterangan tidak mampu tidak ditemukan atau bukan milik anda'
            ], 404);
        }
        ...
        $validated = $request->validate([
           ...
           'no_surat_pengantar' => [
                    'nullable',
                    'regex:/^\d+\/\d+\/\d+$/',
                    function ($attribute, $value, $fail) use ($sktm) {

                        $existing = RtPengantarNumber::where('no_surat_pengantar', $value)
    ->where('id', '!=', optional(
        RtPengantarNumber::where('surat_id', $sktm->id_sktm)
            ->where('surat_type', 'SKTM')
...
if ($existing) {
    ...
    $tujuan = $sktm->rejected_by;
        $keteranganTolak = $sktm->keterangan ? strtolower($sktm->keterangan) : '';
        ...
        $tujuanLabel = match ($tujuan) {
            ...
            // RT validation tetap
            // $validated['rt_validated_at'] = $sktm->rt_validated_at;
            $validated['rt_validated_by'] = $sktm->rt_validated_by;
        ...
        // TAMBAHAN BARU: JIKA DITOLAK OLEH SEKRETARIS LURAH
            ...
            $validated['rt_id'] = $sktm->rt_id;
            $validated['perangkat_id'] = $sktm->perangkat_id;

            $validated['rt_validated_by'] = $sktm->rt_validated_by;
            $validated['perangkat_validated_at'] = $sktm->perangkat_validated_at;
            $validated['perangkat_validated_by'] = $sktm->perangkat_validated_by;
        ...
        if ($request->hasFile('file')) {
            ...
            $validated['file'] = 'uploads/sktm/'.$filename;
        }
       // =====================================================================
        ...
        if ($request->hasFile('file_ktp')) {
            ...
            if ($sktm->file_ktp && file_exists(storage_path('app/' . $sktm->file_ktp))) {
                unlink(storage_path('app/' . $sktm->file_ktp));
            }
            ...
            $validated['file_ktp'] = 'private/sktm/ktp/' . $filenameKtp;
        }
        ...
        if ($request->hasFile('file_kk')) {
            // Hapus file terenkripsi lama di storage private jika ada
            if ($sktm->file_kk && file_exists(storage_path('app/' . $sktm->file_kk))) {
                unlink(storage_path('app/' . $sktm->file_kk));
            }
            ...
            $validated['file_kk'] = 'private/sktm/kk/' . $filenameKk;
        }
       ...
       // =====================================================================
        ...
        if ($request->hasFile('file_pengantar_rt')) {
            ...
            if ($sktm->file_pengantar_rt && file_exists(storage_path('app/' . $sktm->file_pengantar_rt))) {
                unlink(storage_path('app/' . $sktm->file_pengantar_rt));
            }
            ...
            $validated['file_pengantar_rt'] = 'private/pengantar-rt-sktm/' . $filenamePengantar;
            $validated['pengantar_rt_type'] = 'offline';
        }
        ...
        // 🔥 HAPUS FILE LAMA
        if ($sktm->ttd_masyarakat && file_exists(public_path($sktm->ttd_masyarakat))) {
            unlink(public_path($sktm->ttd_masyarakat));
        }
        ...
        $folder = public_path('uploads/sktm/ttd/masyarakat');
                ...
                $validated['ttd_masyarakat'] = 'uploads/sktm/ttd/masyarakat/' . $filenameTtd;
            }
      ...
      // =====================================================
        // 🔥 NOTIFIKASI DINAMIS (PENYESUAIAN ALUR BARU)
        // =====================================================
        $pengaju = $sktm->submitted_by === 'rt' ? 'RT' : 'masyarakat';
        ...
        if ($tujuan === 'rt') {
            Notification::create([
                ...
                'surat_type' => 'SKTM',
                'surat_id'   => $sktm->id_sktm,
                'title'      => 'Ajuan Ulang surat keterangan tidak mampu',
                'message'    => "Terdapat ajuan ulang surat keterangan tidak mampu dari {$pengaju}.",
            ]);
        }
        ...
        // TAMBAHAN NOTIFIKASI SEKRETARIS LURAH
        else if ($tujuan === 'sekretaris_lurah') {
            Notification::create([
                'role'       => 'kepala_desa', // Dikirim ke dashboard pimpinan agar dibaca Sekkel
                'surat_type' => 'SKTM',
                'surat_id'   => $sktm->id_sktm,
                'title'      => 'Ajuan Ulang SKTM ke Sekretaris Lurah',
                'message'    => "Surat keterangan tidak mampu telah diajukan ulang oleh {$pengaju} menuju Sekretaris Lurah.",
            ]);
        }

        else if ($tujuan === 'kepala_desa') {
            Notification::create([
                'role'       => 'kepala_desa', // Dikirim ke dashboard pimpinan agar dibaca Lurah
                'surat_type' => 'SKTM',
                'surat_id'   => $sktm->id_sktm,
                'title'      => 'Ajuan Ulang SKTM ke Lurah',
                'message'    => "Surat keterangan tidak mampu telah diajukan ulang oleh {$pengaju} menuju Lurah.",
            ]);
        }
...
if (!empty($validated['no_surat_pengantar'])) {
    ...
    $existingPengantar = RtPengantarNumber::where('surat_id', $sktm->id_sktm)
        ->where('surat_type', 'SKTM')
        ->first();
...
if ($check->count() > 1) {
    ...
    if ($isSameNumber && $isSameRt && $isSameNamaRt) {
        ...
        return response()->json([
            'success' => true,
            'message' => "Permintaan berhasil diajukan kembali ke {$tujuanLabel}",
            'data' => $sktm->fresh()
        ]);
    ...
    // ✅ PUT: KETUA RT UBAH STATUS & KETERANGAN SKTM
    public function validasiKetuaRT(Request $request, $rt_id, $id_sktm)
{
    // ============================
    ...
    // ============================
    // 3. Ambil data SKTM pemohon
    // ============================
    $sktm = SuketTidakMampu::where('id_sktm', $id_sktm)
        ->where('rt_id', $rt->id)
        ->first();
...
// ==========================================
// 3.5 Validasi status SKTM (RT tidak boleh proses ulang)
// ==========================================
if ($sktm->status === 'ditolak') {
    return response()->json([
        'success' => false,
...
if ($sktm->status === 'diproses perangkat desa') {
    return response()->json([
        'success' => false,
...
if ($sktm->status === 'diproses kepala desa') {
    return response()->json([
        'success' => false,
...
$validated = $request->validate(
 ...
 $updateData = [
    ...
    'keterangan'      => $validated['keterangan'] ?? $sktm->keterangan,
    'rt_validated_at' => now(),
];
...
// ============================
    ...
    if ($request->hasFile('ttd_rt')) {
        ...
        $updateData['ttd_rt'] = 'uploads/sktm/ttd/rt/' . $filename;
    }
...
if ($validated['status'] === 'diproses perangkat desa') {
   ...
   // ============================
    // 7. Update SKTM ke database
    // ============================
    $sktm->update($updateData);
...
if ($validated['status'] === 'diproses perangkat desa') {
    try {
        $pdfUrl = $this->generatePengantarRT($sktm->fresh());
...
if ($validated['status'] === 'diproses perangkat desa') {
    Notification::create([
        ...
        'surat_type' => 'SKTM',
        'surat_id'   => $sktm->id_sktm,
        'title'      => 'Pengajuan surat keterangan tidak mampu Menunggu Verifikasi',
        'message'    => 'Pengajuan surat keterangan tidak mampu telah disetujui Ketua RT.',
    ]);
}
...
if ($validated['status'] === 'diproses perangkat desa') {
    Notification::create([
        'masyarakat_id' => $sktm->masyarakat_id,
        'role'          => 'masyarakat',
        'surat_type'    => 'SKTM',
        'surat_id'      => $sktm->id_sktm,
        'title'         => 'Pengajuan surat keterangan tidak mampu Diproses Ketua RT',
        'message'       => 'Pengajuan surat keterangan tidak mampu Anda telah disetujui Ketua RT dan diteruskan ke Perangkat Desa.',
    ]);
}
...
if ($validated['status'] === 'ditolak') {
    Notification::create([
        ...
        'surat_type'    => 'SKTM',
        'surat_id'      => $sktm->id_sktm,
        'title'         => 'Pengajuan surat keterangan tidak mampu Ditolak RT',
        'message'       => 'Pengajuan surat keterangan tidak mampu Anda ditolak oleh Ketua RT. Silakan periksa keterangan untuk melihat perbaikan.',
    ]);
}
   ...
   // ============================
    ...
    // ============================
    $sktmFresh = $sktm->fresh();

    if (!empty($sktmFresh->ttd_rt)) {
        $sktmFresh->ttd_rt = url($sktmFresh->ttd_rt);
    }

    // 🔥 TAMBAHAN JALUR SECURE API STREAM UNTUK RESPONS DATA FRESH
    if (!empty($sktmFresh->file_ktp)) {
        $sktmFresh->file_ktp = url("api/sktm/file/{$sktmFresh->id_sktm}/ktp");
    }
    if (!empty($sktmFresh->file_kk)) {
        $sktmFresh->file_kk = url("api/sktm/file/{$sktmFresh->id_sktm}/kk");
    }
    if (!empty($sktmFresh->file_pengantar_rt)) {
        $sktmFresh->file_pengantar_rt = url("api/sktm/file/{$sktmFresh->id_sktm}/pengantar");
    }
...
// ✅ PUT: PERANGKAT DESA UBAH NOMOR SURAT, STATUS, & KETERANGAN SKTM
public function validasiPerangkatDesa(Request $request, $pd_id, $id_sktm)
{
    try {
        $pd = auth()->user();
        ...
        $sktm = SuketTidakMampu::where('id_sktm', $id_sktm)->first();
        ...
        if (in_array($sktm->status, ['diproses sekretaris lurah', 'diproses kepala desa'])) {
            return response()->json([
                'success' => false,
        ...
        if (!empty($validated['nomor_surat'])) {
            $existing = SuketTidakMampu::where('nomor_surat', $validated['nomor_surat'])
        ...
        $updateData = [
            ...
            'poin_ii'                => $validated['poin_ii'] ?? $sktm->poin_ii,
            'file_pdf'               => null,
        ];
        ...
        if ($validated['status'] === 'diproses sekretaris lurah') {
            Notification::create([
                ...
                'surat_type' => 'SKTM',
                'surat_id'   => $sktm->id_sktm,
                'title'      => 'SKTM Menunggu Persetujuan Sekretaris Lurah',
                'message'    => 'Pengajuan SKTM menunggu verifikasi dari Sekretaris Lurah.',
            ]);
            ...
            Notification::create([
                ...
                'surat_type'    => 'SKTM',
                'surat_id'      => $sktm->id_sktm,
                'title'         => 'SKTM Diteruskan ke Sekretaris Lurah',
                'message'       => 'Pengajuan SKTM Anda telah disetujui oleh Perangkat Desa dan diteruskan ke Sekretaris Lurah.',
            ]);
        }
        ...
        if ($validated['status'] === 'ditolak') {
            Notification::create([
                ...
                'surat_type'    => 'SKTM',
                'surat_id'      => $sktm->id_sktm,
                'title'         => 'Pengajuan SKTM Ditolak',
                'message'       => 'Pengajuan SKTM Anda ditolak oleh Perangkat Desa. Silakan periksa keterangan untuk melihat perbaikan.',
            ]);
        }
    ...
    // ✅ PUT: KEPALA DESA UBAH STATUS SELESAI & KETERANGAN SKTM
    // ❌ SKTM yang sudah selesai seharusnya TIDAK boleh diproses ulang (Keucali Super Admin)
public function validasiKepalaDesa(Request $request, $kd_id, $id_sktm)
{
    try {
        ...
        $currentStatus = trim($sktm->status);
        ...
        if ($currentStatus === 'diproses sekretaris lurah') {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Status SKTM saat ini tidak valid untuk diproses pada tahap Anda (' . $currentStatus . ').'
            ], 422);
        }
      ...
      // 6. Jika DITOLAK (Baik oleh Sekretaris Lurah maupun Lurah)
        ...
        if ($jabatanUser === 'lurah' && $newStatus === 'selesai') {
            ...
            if ($request->hasFile('ttd_kades')) {
                ...
                $updateData['ttd_kades'] = 'uploads/sktm/ttd/kades/' . $filename;
            } else {
        ...
        if ($jabatanUser === 'lurah' && $newStatus === 'selesai') {
            try {
                $pdfUrl = $this->generatePdf($sktmFresh);
                $sktmFresh->update([
                    'file_pdf' => $pdfUrl
                ]);
            } catch (\Exception $e) {
                Log::error('Gagal generate PDF SKTM oleh Lurah', [
                    'error' => $e->getMessage()
                ]);
            }
        }
        ...
        if (!empty($sktmFresh->ttd_kades)) {
            $sktmFresh->ttd_kades = url($sktmFresh->ttd_kades);
        }
        if (!empty($sktmFresh->file_ktp)) {
            $sktmFresh->file_ktp = url("api/sktm/file/{$sktmFresh->id_sktm}/ktp");
        }
        ...
        if (!empty($sktmFresh->file_pdf)) {
            $sktmFresh->file_pdf = url("api/sktm/file/{$sktmFresh->id_sktm}/pdf");
        }
        ...
        if ($jabatanUser === 'sekretaris lurah') {
            if ($newStatus === 'diproses kepala desa') {
                Notification::create([
                    ...
                    'surat_type' => 'SKTM',
                    'surat_id'   => $sktmFresh->id_sktm,
                    'title'      => 'SKTM Menunggu Persetujuan Lurah',
                    'message'    => 'Pengajuan SKTM telah disetujui oleh Sekretaris Lurah dan kini menunggu persetujuan akhir dari Lurah.',
                ]);
                ...
                Notification::create([
                    ...
                    'surat_type'    => 'SKTM',
                    'surat_id'      => $sktmFresh->id_sktm,
                    'title'         => 'SKTM Diteruskan ke Lurah',
                    'message'       => 'Pengajuan SKTM Anda telah disetujui oleh Sekretaris Lurah dan diteruskan ke Lurah.',
                ]);
            } elseif ($newStatus === 'ditolak') {
                Notification::create([
                    'masyarakat_id' => $sktmFresh->masyarakat_id, // ✅ FIX: diubah dari $sktm ke $sktmFresh
                    'role'          => 'masyarakat',
                    'surat_type'    => 'SKTM',
                    'surat_id'      => $sktmFresh->id_sktm, // ✅ FIX: diubah dari $sktm ke $sktmFresh
                    'title'         => 'SKTM Ditolak Sekretaris Lurah',
                    'message'       => 'Pengajuan SKTM Anda ditolak oleh Sekretaris Lurah. Silakan periksa keterangan.',
                ]);
            }
        } 
      ...
      if ($jabatanUser === 'lurah') {
            if ($newStatus === 'selesai') {
                Notification::create([
                    ...
                    'surat_id'      => $sktmFresh->id_sktm,
                    'title'         => 'SKTM Selesai',
                    'message'       => 'Pengajuan SKTM Anda telah disetujui oleh Lurah dan surat sudah dapat diunduh.',
                ]);
            } elseif ($newStatus === 'ditolak') {
                Notification::create([
                    'masyarakat_id' => $sktmFresh->masyarakat_id, // ✅ FIX: diubah dari $sktm ke $sktmFresh
                    ...
                    'surat_type'    => 'SKTM',
                    'surat_id'      => $sktmFresh->id_sktm,
                    'title'         => 'SKTM Ditolak Lurah',
                    'message'       => 'Pengajuan SKTM Anda ditolak oleh Lurah. Silakan periksa keterangan untuk melihat perbaikan.',
                ]);
            }
        }
        ...
        if ($jabatanUser === 'sekretaris lurah') {
            $returnMessage = $newStatus === 'diproses kepala desa' ? 'SKTM berhasil diverifikasi dan diteruskan ke Lurah.' : 'SKTM ditolak oleh Sekretaris Lurah.';
        } elseif ($jabatanUser === 'lurah') {
            $returnMessage = $newStatus === 'selesai' ? 'SKTM selesai dan PDF berhasil dibuat.' : 'SKTM ditolak oleh Lurah.';
        }
    ...
    public function exportSKTM(Request $request)
    {
        $user = $request->user();
        ...
        if (!in_array($user->role, ['perangkat_desa', 'super_admin'])) {
            return response()->json([
                'success' => false,
                'message' => 'Anda tidak memiliki akses untuk export data SKTM.',
            ], 403);
        }
    ...
    public function exportZipSKTM(Request $request)
    {
        $user = $request->user();
        ...
        $zipFileName = 'SKTM_' . $safeFilename . '_' . now()->format('Y-m-d_H-i-s') . '.zip';
        ...
        if ($zip->open($zipPath, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            ...
            // 📄 FILE SKTM (DARI DB)
            // ===========================
            $data = SuketTidakMampu::whereNotNull('file_pdf')->get();
            ...
            // ===========================
            $folderPengantar = public_path('uploads/pengantar-rt-sktm');
    ...
    public function deleteAllSKTM(Request $request)
    {
        try {
            $user = $request->user();
            ...
            // update deleted_by
            SuketTidakMampu::query()->update([
                'deleted_by' => $deletedByRole,
            ]);

            // delete database
            SuketTidakMampu::query()->delete();
   ...
   public function generatePdf($sktm)
    {
        try {
            // 🔥 1. Pindahkan folder tujuan ke private storage_path
            $pdfFolder = storage_path('app/private/sktm/pdf');
            ...
            $pdf = Pdf::loadView('pdf.sktm', ['data' => $sktm])
                ->setPaper('A4', 'portrait');

            // 🔥 2. Buat nama file sangat acak menggunakan Str::random(40)
            $randomName = \Illuminate\Support\Str::random(40);
            $filename = 'sktm_' . $randomName . '.pdf';
            ...
            return 'private/sktm/pdf/' . $filename;
    ...
    public function generatePengantarRT($sktm)
{
    try {
        // 🔥 1. Pindahkan folder tujuan ke private storage_path
        $pdfFolder = storage_path('app/private/pengantar-rt-sktm');
        ...
        $pdf = PDF::loadView('pdf.pengantar_sktm', [
                'sktm' => $sktm
            ])
            ->setPaper('A4', 'portrait');
        ...
        // 🔥 4. Kembalikan relative path untuk disimpan di database
        return 'private/pengantar-rt-sktm/' . $filename;
