...
class SuketJandaController extends Controller
{

 public function countSKJ(Request $request)
    {
        try {
            // ===============================
            // 1. VALIDASI TOKEN
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil jumlah Surat Keterangan Janda',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function index()
    {
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat mengambil data Surat Keterangan Janda',
                'error' => $e->getMessage()
            ], 500);
        }
    }

// GET DATA Surat Keterangan Janda BY MASYARAKAT (TOKEN + VALIDASI ID) baru diperbaiki
public function getByMasyarakat(Request $request)
{
    try {
      ...
      $masyarakat = $request->user();
        ...
        $paginator = SuketJanda::where('masyarakat_id', $tokenMasyarakatId)
            ->with([
...
Log::info('Surat Keterangan Janda PAGINATION DEBUG', [
    ...
    'last_page'    => $paginator->lastPage(),
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar pengajuan Surat Keterangan Janda masyarakat',
    ...
    // GET DATA Surat Keterangan Janda BY KETUA RT TOKEN LOGIN
   public function getByKetuaRT(Request $request)
{
    try {
        ...
        // Query + pagination
        $data = SuketJanda::where('rt_id', $rt->id)
            ->with([
                'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar Surat Keterangan Janda untuk RT yang sedang login',
            'pagination' => [
    ...
    // GET DATA Surat Keterangan Janda BY PERANGKAT DESA TOKEN LOGIN
public function getByPerangkatDesa(Request $request)
{
    try {
        ...
        // Perangkat desa bisa melihat semua Surat Keterangan Janda
        $data = SuketJanda::with([
                'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar Surat Keterangan Janda untuk perangkat desa',
            'pagination' => [
                'current_page' => $data->currentPage(),
                'per_page'     => $data->perPage(),
                'total'        => $data->total(),
    ...
    // GET DATA Surat Keterangan Janda BY KEPALA DESA TOKEN LOGIN
public function getByKepalaDesa(Request $request)
{
    try {
        ...
        // Kepala desa bisa melihat semua Surat Keterangan Janda
        $data = SuketJanda::with([
                'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Daftar Surat Keterangan Janda untuk kepala desa',
            'pagination' => [
                'current_page' => $data->currentPage(),
                'per_page'     => $data->perPage(),
                'total'        => $data->total(),
    ...
    // GET DATA Surat Keterangan Janda BY SUPER ADMIN TOKEN LOGIN
    public function getAll(Request $request)
    {
        try {
            ...
            // Super admin bisa melihat semua Surat Keterangan Janda
           $data = SuketJanda::with([
               'rt:id,nama,jabatan,role',
                    'perangkat:id,nama,jabatan,role',
                    'perangkatValidator:id,nama,jabatan,role',
            ...
            return response()->json([
                'success' => true,
                'message' => 'Daftar Surat Keterangan Janda (super admin)',
                'count' => $data->count(),
                'data' => $data
            ]);
    ...
    // POST DATA AJUAN Surat Keterangan Janda BY MASYARAKAT
   public function store(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'nama_mantan_suami' => 'required|string',
     ...
     if ((int) $validated['masyarakat_id'] !== (int) $masyarakat->id) {
        return response()->json([
            'success' => false,
            'message' => 'Anda tidak berhak mengajukan Surat Keterangan Janda untuk masyarakat lain'
        ], 403);
    }
    ...
    if ($request->hasFile('file')) {
        $folder = public_path('uploads/skj');
        ...
        $validated['file'] = url('uploads/skj/' . $filename);
    }
    ...
    if ($request->hasFile('file_ktp')) {
        $folder = public_path('uploads/skj/ktp');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['file_ktp'] = 'uploads/skj/ktp/' . $filename;
    }
    ...
    if ($request->hasFile('file_kk')) {
        $folder = public_path('uploads/skj/kk');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['file_kk'] = 'uploads/skj/kk/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_masyarakat')) {
        $folder = public_path('uploads/skj/ttd/masyarakat');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_masyarakat'] = 'uploads/skj/ttd/masyarakat/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_rt')) {
        $folder = public_path('uploads/skj/ttd/rt');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_rt'] = 'uploads/skj/ttd/rt/' . $filename;
    }
    ...
    if ($request->hasFile('ttd_kades')) {
        $folder = public_path('uploads/skj/ttd/kades');
        if (!file_exists($folder)) mkdir($folder, 0777, true);
        ...
        $validated['ttd_kades'] = 'uploads/skj/ttd/kades/' . $filename;
    }
        ...
        $skj = SuketJanda::create($validated);
    ...
    Notification::create([
        ...
        'surat_type' => 'SKJ',
        'surat_id'   => $skj->id_skj,
        'title'      => 'Pengajuan Surat Keterangan Janda',
        'message'    => 'Ada pengajuan Surat Keterangan Janda baru dari masyarakat.',
    ]);

        return response()->json([
            'success' => true,
            'message' => 'Pengajuan Surat Keterangan Janda berhasil dibuat',
            'data' => $skj->load(['rt:id,nama,role,no_rt', 'masyarakat:id,nama,email,nik']),
        ], 201);
    }
        ...
        public function storeByRT(Request $request)
{
    ...
    Notification::create([
        ...
        'surat_type' => 'SKJ',
        'surat_id'   => $skj->id_skj,
        'title'      => 'Pengajuan Surat Keterangan Janda dari RT',
        'message'    => 'RT telah mengajukan dan memverifikasi Surat Keterangan Janda.',
    ]);

    return response()->json([
        'success' => true,
        'message' => 'Pengajuan Surat Keterangan Janda oleh RT berhasil',
        'data' => $skj->load(['rt:id,nama,no_rt']),
    ], 201);
}
...
if (!empty($validated['rt_id'])) {
    ...
    return response()->json([
        ...
        if ($request->hasFile('file_pengantar_rt')) {
            $folder = public_path('uploads/pengantar-rt-skj');
            if (!file_exists($folder)) mkdir($folder, 0777, true);
            ...
            $validated['file_pengantar_rt'] = 'uploads/pengantar-rt-skj/' . $filename;
        ...
        Notification::create([
            ...
            'surat_type' => 'SKJ',
            'surat_id'   => $skj->id_skj,
            'title' => 'Ajuan Surat Keterangan Janda Menunggu verifikasi Perangkat Desa',
            'message' => 'Pengajuan Surat Keterangan Janda dari perangkat desa (dengan pengantar RT offline) menunggu verifikasi.',
        ]);
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pengajuan Surat Keterangan Janda oleh perangkat desa berhasil',
            'data' => $skj
        ], 201);
    }
    ...
    public function ajukanKembali(Request $request, $id)
{
     try {
        ...
        if (!$user) {
         ...
         // === Ambil SKJ ===
        $skj = SuketJanda::where('id_skj', $id)->first();

        // ✅ FIX: cek dulu sebelum dipakai
        if (!$skj) {
            return response()->json([
                'success' => false,
                'message' => 'Data Surat Keterangan Janda tidak ditemukan'
            ], 404);
        }

            // === CEK HAK AKSES BERDASARKAN PENGAJU ===
        if ($skj->submitted_by === 'masyarakat') {

            if (!$user->tokenCan('masyarakat') || $skj->masyarakat_id != $user->id) {
                return response()->json([
                    'success' => false,
        ...
        } else if ($skj->submitted_by === 'rt') {

            if (!$user->tokenCan('rt') || $skj->rt_id != $user->id) {
                return response()->json([
                    'success' => false,
        ...
        } else if ($skj->submitted_by === 'perangkat_desa') {
    ...
    // 🔥 FIX DI SINI
    if (!$request->hasFile('file_pengantar_rt') && !$skj->file_pengantar_rt) {
        return response()->json([
            'success' => false,
    ...
    $validated['rt_validated_at'] = now();
        ...
        if (!$skj) {
            return response()->json([
                'success' => false,
                'message' => 'Data Surat Keterangan Janda tidak ditemukan atau bukan milik anda'
            ], 404);
        }

        if ($skj->status !== 'ditolak') {
            return response()->json([
                'success' => false,
    ...
    'nullable',
    'regex:/^\d+\/\d+\/\d+$/',
    function ($attribute, $value, $fail) use ($skj) {

        $existing = RtPengantarNumber::where('no_surat_pengantar', $value)
    ->where('id', '!=', optional(
        RtPengantarNumber::where('surat_id', $skj->id_skj)
            ->where('surat_type', 'SKJ')
    ...
    if ($request->filled('rt_id')) {
       ...
       // Ambil RT baru jika diubah
        if ($request->filled('rt_id')) {
            ...
            $validated['no_rt'] = $skj->no_rt; // tetap jika tidak diubah
        }
...
$validated['rt_validated_at'] = now();
            ...
            // RT validation tetap
            // $validated['rt_validated_at'] = $skj->rt_validated_at;
            $validated['rt_validated_by'] = $skj->rt_validated_by;
            ...
            $validated['rt_id'] = $skj->rt_id;
            $validated['perangkat_id'] = $skj->perangkat_id;
            ...
            // perangkat tetap
            $validated['perangkat_validated_at'] = $skj->perangkat_validated_at;
            $validated['perangkat_validated_by'] = $skj->perangkat_validated_by;
        ...
        if ($request->hasFile('file')) {
            ...
            $validated['file'] = 'uploads/skj/'.$filename;
        }
        // ============================
        ...
        // 🔥 HAPUS FILE LAMA
        if ($skj->file_ktp && file_exists(public_path($skj->file_ktp))) {
            unlink(public_path($skj->file_ktp));
        }
        ...
        $folder = public_path('uploads/skj/ktp');
                ...
                $validated['file_ktp'] = 'uploads/skj/ktp/' . $filenameKtp;
            }
        ...
        // 🔥 HAPUS FILE LAMA
        if ($skj->file_kk && file_exists(public_path($skj->file_kk))) {
            unlink(public_path($skj->file_kk));
        }
        ...
        $folder = public_path('uploads/skj/kk');
                ...
                $validated['file_kk'] = 'uploads/skj/kk/' . $filenameKk;
            }

        if ($request->hasFile('file_pengantar_rt')) {

        // 🔥 NORMALISASI PATH
        $oldPath = $skj->file_pengantar_rt;
        ...
        if (!file_exists($fullPath)) {
            $fullPath = public_path(str_replace('uploads/pengantar-rt-skj', 'uploads/pengantar-rt-skj/', $oldPath));
        }
        ...
        $validated['file_pengantar_rt'] = 'uploads/pengantar-rt-skj/' . $filenamePengantar;
        ...
        // 🔥 HAPUS FILE LAMA
        if ($skj->ttd_masyarakat && file_exists(public_path($skj->ttd_masyarakat))) {
            unlink(public_path($skj->ttd_masyarakat));
        }
        ...
        $folder = public_path('uploads/skj/ttd/masyarakat');
                ...
                $validated['ttd_masyarakat'] = 'uploads/skj/ttd/masyarakat/' . $filenameTtd;
            }
    ...
    Notification::create([
        ...
        'surat_type' => 'SKJ',
        'surat_id'   => $skj->id_skj,
        'title'      => 'Ajuan Ulang Surat Keterangan Janda',
        'message'    => 'Terdapat ajuan ulang Surat Keterangan Janda dari masyarakat untuk diproses kembali.',
    ]);
}
    ...
    Notification::create([
        ...
        'surat_id'   => $skj->id_skj,
        'title'      => 'Ajuan Ulang Surat Keterangan Janda',
        'message'    => 'Surat Keterangan Janda yang sebelumnya ditolak telah diajukan ulang oleh masyarakat.',
    ]);
}
...
if (!empty($validated['no_surat_pengantar'])) {
    ...
    $existingPengantar = RtPengantarNumber::where('surat_id', $skj->id_skj)
        ->where('surat_type', 'SKJ')
        ->first();
...
if ($check->count() > 1) {
 ...
 // update hanya kalau berubah
    ...
    // ✅ PUT: KETUA RT UBAH STATUS & KETERANGAN Surat Keterangan Janda
        public function validasiKetuaRT(Request $request, $rt_id, $id_skj)
{
    // ============================
    ...
    // ============================
    // 3. Ambil data SKJ pemohon
    // ============================
    $skj = SuketJanda::where('id_skj', $id_skj)
        ->where('rt_id', $rt->id)
        ->first();
...
// ==========================================
// 3.5 Validasi status SKJ (RT tidak boleh proses ulang)
// ==========================================
if ($skj->status === 'ditolak') {
    return response()->json([
        'success' => false,
...
if ($skj->status === 'diproses perangkat desa') {
    return response()->json([
        'success' => false,
...
if ($skj->status === 'diproses kepala desa') {
    return response()->json([
        'success' => false,
...
$validated = $request->validate(
 ...
 $updateData = [
    ...
    'keterangan'      => $validated['keterangan'] ?? $skj->keterangan,
    'rt_validated_at' => now(),
];
...
// ============================
    ...
    if ($request->hasFile('ttd_rt')) {
        ...
        $updateData['ttd_rt'] = 'uploads/skj/ttd/rt/' . $filename;
    }
...
if ($validated['status'] === 'diproses perangkat desa') {
   ...
   // ============================
    // 7. Update skj ke database
    // ============================
    $skj->update($updateData);
...
if ($validated['status'] === 'diproses perangkat desa') {
    try {
        $pdfUrl = $this->generatePengantarRT($skj->fresh());
...
if ($validated['status'] === 'diproses perangkat desa') {
    Notification::create([
        ...
        'surat_type' => 'SKJ',
        'surat_id'   => $skj->id_skj,
        'title'      => 'SKJ Menunggu Verifikasi',
        'message'    => 'Pengajuan SKJ telah divalidasi RT.',
    ]);
}
...
if ($validated['status'] === 'diproses perangkat desa') {
    Notification::create([
        'masyarakat_id' => $skj->masyarakat_id,
        'role'          => 'masyarakat',
        'surat_type'    => 'SKJ',
        'surat_id'      => $skj->id_skj,
        'title'         => 'Pengajuan Surat Keterangan Janda Diverifikasi RT',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda telah diverifikasi RT dan diteruskan ke Perangkat Desa.',
    ]);
}
...
if ($validated['status'] === 'ditolak') {
    Notification::create([
        ...
        'surat_type'    => 'SKJ',
        'surat_id'      => $skj->id_skj,
        'title'         => 'Pengajuan Surat Keterangan Janda Ditolak RT',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda ditolak oleh RT. Silakan periksa keterangan.',
    ]);
}
    ...
    if (!empty($skjFresh->ttd_rt)) {
        $skjFresh->ttd_rt = url($skjFresh->ttd_rt);
    }
    ...
    // ✅ PUT: PERANGKAT DESA UBAH NOMOR SURAT, STATUS, & KETERANGAN Surat Keterangan Janda
   public function validasiPerangkatDesa(Request $request, $pd_id, $id_skj)
{
    try {
        ...
        // ================================
        // 3. Ambil data Surat Keterangan Janda
        // ================================
        $skj = SuketJanda::where('id_skj', $id_skj)->first();
    ...
    return response()->json([
       ...
       $validated = $request->validate([
        ...
        if (!empty($validated['nomor_surat'])) {
            $existing = SuketJanda::where('nomor_surat', $validated['nomor_surat'])
                ->where('id_skj', '!=', $id_skj)
                ->first();
        ...
        $updateData = [
            ...
            'poin_ii' => $validated['poin_ii'] ?? $skj->poin_ii,
            'file_pdf' => null,
        ];
...
if ($validated['status'] === 'diproses kepala desa') {
    Notification::create([
        ...
        'surat_type' => 'SKJ',
        'surat_id'   => $skj->id_skj,
        'title'      => 'Surat Keterangan Janda Menunggu Persetujuan',
        'message'    => 'Pengajuan Surat Keterangan Janda menunggu persetujuan Kepala Desa.',
    ]);
}
// 🔔 NOTIFIKASI KE MASYARAKAT
if ($validated['status'] === 'diproses kepala desa') {
    Notification::create([
        ...
        'surat_type'    => 'SKJ',
        'surat_id'      => $skj->id_skj,
        'title'         => 'Surat Keterangan Janda Diteruskan ke Kepala Desa',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda telah diverifikasi oleh Perangkat Desa dan diteruskan ke Kepala Desa.',
    ]);
}
...
if ($validated['status'] === 'ditolak') {
    Notification::create([
        ...
        'surat_type'    => 'SKJ',
        'surat_id'      => $skj->id_skj,
        'title'         => 'Pengajuan Surat Keterangan Janda Ditolak',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda ditolak oleh Perangkat Desa. Silakan periksa keterangan.',
    ]);
}
        ...
        // ) {
        //     try {
        //         $pdfUrl = $this->generatePdf($skj);
        //         $skj->update(['file_pdf' => $pdfUrl]);
        //     } catch (\Exception $e) {
    ...
    // ✅ PUT: KEPALA DESA UBAH STATUS SELESAI & KETERANGAN Surat Keterangan Janda
    // ❌ Surat Keterangan Janda yang sudah selesai seharusnya TIDAK boleh diproses ulang (Keucali Super Admin)
    public function validasiKepalaDesa(Request $request, $kd_id, $id_skj)
    {
        try {
            ...
            if (!$skj) {
                return response()->json([
                    'success' => false,
                    'message' => 'Data SKJ tidak ditemukan.'
                ], 404);
            }
...
if ($currentStatus === 'ditolak') {
    return response()->json([
        'success' => false,
        'message' => 'Surat Keterangan Janda telah ditolak. Silakan tunggu hingga diajukan ulang oleh pemohon.'
    ], 422);
}
...
if ($currentStatus === 'diproses perangkat desa') {
    return response()->json([
        'success' => false,
        'message' => 'Surat Keterangan Janda masih diproses oleh Perangkat Desa.'
    ], 422);
}
...
if ($currentStatus === 'diproses rt') {
    return response()->json([
        'success' => false,
        'message' => 'Surat Keterangan Janda masih diproses oleh Ketua RT.'
    ], 422);
}
...
if ($currentStatus !== 'diproses kepala desa') {
    return response()->json([
        'success' => false,
        'message' => 'Status Surat Keterangan Janda tidak valid untuk diproses oleh Kepala Desa.'
    ], 422);
}
            ...
            if ($newStatus === 'selesai' && $request->hasFile('ttd_kades')) {
                ...
                $updateData['ttd_kades'] = 'uploads/skj/ttd/kades/' . $filename;
            }
...
if ($newStatus === 'selesai') {
    ...
    // ⚠️ Jika upload file → jangan timpa
            ...
            if ($newStatus === 'selesai') {
                try {
                    $pdfUrl = $this->generatePdf($skjFresh);
                    $skjFresh->update([
                        'file_pdf' => $pdfUrl
                    ]);
                } catch (\Exception $e) {
                    Log::error('Gagal generate PDF Surat Keterangan Janda oleh Kepala Desa', [
                        'error' => $e->getMessage()
                    ]);
                }
            }
            ...
            // 10. Format URL TTD
            if (!empty($skjFresh->ttd_kades)) {
                $skjFresh->ttd_kades = url($skjFresh->ttd_kades);
            }

            return response()->json([
                'success' => true,
                'message' => $newStatus === 'selesai'
                    ? 'Surat Keterangan Janda selesai dan PDF berhasil dibuat.'
                    : 'Surat Keterangan Janda berhasil ditolak.',
                'data' => $skjFresh
            ]);
...
if ($newStatus === 'selesai') {
    Notification::create([
        ...
        'role'          => 'masyarakat',
        'surat_type'    => 'SKJ',
        'surat_id'      => $skjFresh->id_skj,
        'title'         => 'Pengajuan Surat Keterangan Janda Selesai',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda telah disetujui oleh Kepala Desa dan surat sudah dapat diunduh.',
    ]);
}
...
if ($newStatus === 'ditolak') {
    Notification::create([
        ...
        'surat_type'    => 'SKJ',
        'surat_id'      => $skjFresh->id_skj,
        'title'         => 'Surat Keterangan Janda Ditolak',
        'message'       => 'Pengajuan Surat Keterangan Janda Anda ditolak oleh Kepala Desa. Silakan periksa keterangan.',
    ]);
}
...
public function exportSKJ(Request $request)
{
    $user = $request->user();
    ...
    if (!in_array($user->role, ['perangkat_desa', 'super_admin'])) {
        return response()->json([
            'success' => false,
            'message' => 'Anda tidak memiliki akses untuk export data Surat Keterangan Janda.',
        ], 403);
    }
    ...
    $finalFilename = 'Surat Keterangan Janda_' .
        $safeFilename . '_' .
...
public function exportZipSKJ(Request $request)
{
    $user = $request->user();
    ...
    if (!in_array($user->role, ['perangkat_desa', 'super_admin'])) {
        return response()->json([
            'success' => false,
            'message' => 'Anda tidak memiliki akses untuk export data SKJ.',
        ], 403);
    }
    ...
    $zipFileName = 'SKJ_' . $safeFilename . '_' . now()->format('Y-m-d_H-i-s') . '.zip';
    ...
    if ($zip->open($zipPath, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
        ...
        // 📄 FILE SKJ (DARI DB)
        // ===========================
        $data = SuketJanda::whereNotNull('file_pdf')->get();
        ...
        // ===========================
        $folderPengantar = public_path('uploads/pengantar-rt-skj');
...
public function deleteAllSKJ(Request $request)
{
    try {
        $user = $request->user();
        ...
        // update deleted_by
        SuketJanda::query()->update([
            'deleted_by' => $deletedByRole,
        ]);

        // delete database
        SuketJanda::query()->delete();
...
public function searchfilter(Request $request)
{
    ...
    {
        $data = SuketJanda::with(['rt:id,name,role', 'masyarakat:id,nama,email,nik'])->find($id);
        if (!$data) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'nama_mantan_suami' => 'sometimes|string',
        ]);

        // Upload file baru
        if ($request->hasFile('file')) {
            if ($skj->file) {
                $oldPath = str_replace(url('/') . '/', '', $skj->file);
                if (file_exists(public_path($oldPath))) unlink(public_path($oldPath));
            }

            $file = $request->file('file');
            $filename = time() . '_' . $file->getClientOriginalName();
            $file->move(public_path('uploads/skj'), $filename);
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data Surat Keterangan Janda berhasil diperbarui',
            'data' => $skj->load(['rt:id,nama,role', 'masyarakat:id,nama,email,nik'])
        ]);
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Data Surat Keterangan Janda berhasil dihapus']);
    }


    // ✅ GET: /api/rt/{rt_id}/skj
    public function getByRT($rt_id)
    {
        ...
        // Hanya ambil data Surat Keterangan Janda milik RT tersebut, TANPA mengubah status apa pun
        $data = SuketJanda::where('rt_id', $rt_id)
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data Surat Keterangan Janda milik RT berhasil diambil',
            'data' => $data
        ]);
    }

    // ✅ PUT: /api/skj/{id}/tolak
    ...
    public function generatePdf($skj)
    {
        try {
            $pdfFolder = public_path('uploads/skj/pdf');
            ...
            // Muat PDF
            $pdf = Pdf::loadView('pdf.skj', ['data' => $skj])
                ->setPaper('A4', 'portrait');

            $filename = 'skj_' . $skj->id_skj . '.pdf';
            $fullPath = $pdfFolder . '/' . $filename;

            // Simpan ke server
            $pdf->save($fullPath);

            return url('uploads/skj/pdf/' . $filename);
       ...
       public function generatePengantarRT($skj)
{
    try {
        $pdfFolder = public_path('uploads/pengantar-rt-skj');
        ...
        // Muat PDF dari Blade
        $pdf = PDF::loadView('pdf.pengantar_skj', [
                'skj' => $skj
            ])
            ->setPaper('A4', 'portrait');

        // Nama file
        $filename = 'pengantar_rt_skj_' . $skj->id_skj . '.pdf';
        $fullPath = $pdfFolder . '/' . $filename;
        ...
        // URL akses file
        return url('uploads/pengantar-rt-skj/' . $filename);
